...
// Import your local decision_tree.json
import decisionTreeJson from "./decision_tree.json";
...
export default function DecisionTree() {
  ...
  // Instead of fetching from a remote URL, we use the locally imported JSON file.
  React.useEffect(() => {
  ...
  const handleOnChoiceClick = (choice) => {
    ...
    const newHistory = [
      ...stateHistory.slice(0, currentIndexInHistory + 1),
      { nodeStack: new_node_stack, solutionPath: new_solution },
    ];
  ...
  return (
    <Box
      className={layout === uiLayout.WITHOUT_CODE_EDITOR ? "container-without-code-editor" : "container"}
      sx={{ display: "flex" }}
      minHeight="100vh"
      minWidth="100vw"
    >
      ...
      <Box sx={{ position: "fixed", bottom: "10px", right: "10px" }}>
        <IconButton
          ...
          style={{ backgroundColor: layout === uiLayout.WITH_CODE_EDITOR ? "white" : "inherit" }}
        >
      ...
      <Box sx={{ display: "flex", flexDirection: layout === uiLayout.WITHOUT_CODE_EDITOR ? "row" : "column", flexGrow: 1 }}>
        {questionView === "one_by_one" && (
          <Box display="flex" justifyContent="center" alignItems="center" flexGrow={1} maxWidth="50vw">
            <Box sx={{ position: "fixed", top: "10px", left: "10px" }}>
              <Tooltip title="Use AI">
                <GlowingButton
                  is_clicked={useAI ? 1 : 0}
                  onClick={() => {
                    setUseAI(!useAI);
            ...
            {current_node && (
              <Box margin="10px">
                ...
                <Typography textAlign="center" sx={{ marginBottom: "20px" }} variant="h4">
                  {current_node.question}
                </Typography>

                <Box display="flex" flexDirection="column">
                  {current_node.children.map((child) => (
                    <Button color="inherit" key={child.id} onClick={() => handleOnChoiceClick(child)}>
                      {child.label}
                    </Button>
                  ))}
                </Box>
        ...
        <Box
          ...
          className="diagram"
        >
          {questionView === "tree" && <h3>Block diagram not supported in tree view</h3>}
          <pre id="mermaid" style={{ display: questionView === "one_by_one" ? "block" : "none" }}></pre>
        </Box>
      </Box>
      {layout === uiLayout.WITH_CODE_EDITOR && <iframe style={{ width: "100%" }} src={codeSandboxUrl}></iframe>}
    </Box>
  );
}
